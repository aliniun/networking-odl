...
def _generate_loadbalancer_deps(data):
    object_ids = []

    port_ids = data.get('vip_port_id')
    if port_ids is not None:
        object_ids.extend(port_ids)

    return object_ids


def _generate_listener_deps(data):
    object_ids = [loadbalancer['id'] for loadbalancer in data['loadbalancers']]

    return object_ids


def _generate_pool_deps(data):
    object_ids = []

    loadbalancer_ids = [loadbalancer['id'] for loadbalancer in data['loadbalancers']]
    if loadbalancer_ids is not None:
        object_ids.extend(loadbalancer_ids)

    listener_ids = [listener['id'] for listener in data['listeners']]
    if listener_ids is not None:
        object_ids.extend(listener_ids)

    return object_ids


def _generate_healthmonitor_deps(data):
    object_ids = [pool['id'] for pool in data['pools']]

    return object_ids


def _generate_member_deps(data):
    object_ids = []

    pool_ids = data.get('pool_id')
    if pool_ids is not None:
        object_ids.extend(pool_ids)

    return object_ids


def _generate_firewall_deps(data):
    object_ids = []

    fw_policy_id = data.get('fw_policy_id')
    if fw_policy_id is not None:
        object_ids.extend(fw_policy_id)

    return object_ids


def _generate_firewall_policy_deps(data):
    object_ids = []

    fw_rule_ids = data.get('firewall_rule_ids')
    if fw_rule_ids is not None:
        object_ids.extend(fw_rule_ids)
...
_CREATE_OR_UPDATE_DEP_GENERATOR = {
    ...
    odl_const.ODL_LOADBALANCER: _generate_loadbalancer_deps,
    odl_const.ODL_LISTENER: _generate_listener_deps,
    odl_const.ODL_POOL: _generate_pool_deps,
    odl_const.ODL_HEALTHMONITOR: _generate_healthmonitor_deps,
    odl_const.ODL_MEMBER: _generate_member_deps,
    odl_const.ODL_FIREWALL: _generate_firewall_deps,
    odl_const.ODL_FIREWALL_POLICY: _generate_firewall_policy_deps,
}
...
_DELETE_DEPENDENCIES = {
    ...
    odl_const.ODL_ROUTER: (odl_const.ODL_PORT, odl_const.ODL_FLOATINGIP,
                           odl_const.ODL_BGPVPN),
    odl_const.ODL_PORT: (odl_const.ODL_TRUNK, odl_const.ODL_LOADBALANCER,),
    # TODO(yamahata): dependency between SG and PORT
    ...
    odl_const.ODL_LOADBALANCER: (odl_const.ODL_LISTENER, odl_const.ODL_POOL),
    odl_const.ODL_LISTENER: (odl_const.ODL_POOL),
    odl_const.ODL_POOL: (odl_const.ODL_HEALTHMONITOR, odl_const.ODL_MEMBER),
}
